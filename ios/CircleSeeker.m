...
@interface CircleSeeker (private)

@property (readonly, nonatomic) CGFloat radius;

@end
@implementation CircleSeeker {
    int angle;
    int fixedAngle;
...
- (void)defaults {
    // Defaults
    _maximumValue = 360.0f;
    _minimumValue = 0.0f;
    _currentValue = 0.0f;
    _lineWidth = 25;
    _lineWidthBackground = 25;
    _lineRadiusDisplacement = 5;
    _colorCircleBackground = [UIColor blackColor];
    _filledColor = [UIColor redColor];
    _colorPoint = _filledColor;
...
- (void)setValue:(NSNumber *)crr {
    _currentValue=[crr integerValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextAddArc(ctx, self.frame.size.width/2, self.frame.size.height/2, self.frame.size.height/2 - _lineWidthBackground / 2 - _lineRadiusDisplacement, 0, M_PI *2, 0);
    
    [_colorCircleBackground setStroke];
    CGContextSetLineWidth(ctx, _lineWidthBackground);
...
//        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, self.frame.size.height/2 - _lineWidthBackground / 2 - _lineRadiusDisplacement, 3*M_PI/2, 3*M_PI/2-ToRad(angle+3), 0);
//    } else {
        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2,self.frame.size.height/2 - _lineWidthBackground / 2 - _lineRadiusDisplacement, 3*M_PI/2, 3*M_PI/2-ToRad(angle), 0);
...
-(BOOL) beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    [self.bridge.eventDispatcher sendAppEventWithName:@"onDownCircleSeeker" body:@(_currentValue)];
    return YES;
}
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    [self.bridge.eventDispatcher sendAppEventWithName:@"onUpCircleSeeker" body:@(_currentValue)];
}
...
-(void)moveHandle:(CGPoint)point {
    ...
    [self.bridge.eventDispatcher sendAppEventWithName:@"onChangeCircleSeeker" body:@(_currentValue)];
}
...
-(CGPoint)pointFromAngle:(int)angleInt{
    ...
    float rad = self.frame.size.height/2  - _lineRadiusDisplacement - _lineWidthBackground / 2;
    
    //Define The point position on the circumference
    CGPoint result;
    result.y = round(centerPoint.y + rad * sin(ToRad(-angleInt-90))) ;
    result.x = round(centerPoint.x + rad * cos(ToRad(-angleInt-90)));
...
- (void)setWithLine:(NSNumber *)withLine
{
    _lineWidth = [withLine integerValue];
}
- (void)setWithLineBackground:(NSNumber *)withLineBackground
{
    _lineWidthBackground = [withLineBackground integerValue];
}
- (void)setColorCircle:(UIColor *)colorCircle
{
    _filledColor = colorCircle;
}
